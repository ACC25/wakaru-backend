...
describe "GET fixtures" do
  it "responds with an array of fixtures for the user dashboard" do
    ...
    company_2 = create(:company, name: "something else")
    ...
    expect(reply[0]).to eq(good_good_response_1.id)
    expect(reply[1]).to eq(good_good_response_4.id)
    expect(reply[2]).to eq(bad_bad_response_5.id)
    expect(reply.include?(bad_bad_response_7.id)).to eq(false)
  end
end
